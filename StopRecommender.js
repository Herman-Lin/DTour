...
export class StopRecommender {
    ...
    */
    ...
    getStopSuggestion(searchStr, stops, callback) {
        if (searchStr === undefined) {console.log(searchStr); return;}
        if (searchStr.match(/^[\d]+ /i) !== null) {
                        ...
                        Http2.onreadystatechange = e => {
                            if (Http2.readyState == 4 && Http2.status == 200) {
                                ...
                                var result_json = {
                                    ...
                                    "image_url": "https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Ic_pin_drop_48px.svg/500px-Ic_pin_drop_48px.svg.png",
                                    "is_closed": false,
                                    "review_count": 0,
                                    "categories": [{"alias":"address", "title":"address"}],
                                    "rating": 0,
                                    "coordinates": {"latitude": response2.result.geometry.location.lat, "longitude": response2.result.geometry.location.lng},
                                    "price": "$$$$",
                                    "location": {"display_address": [prediction['description']]},
                                };
            ...
            searchQueue.forEach (function (yelpSearchParam) {
                const Http = new XMLHttpRequest();
                var url = "https://api.yelp.com/v3/businesses/search?" + "term=" + encodeURIComponent(yelpSearchParam.term) + "&latitude=" + String(yelpSearchParam.latitude) + "&longitude=" + String(yelpSearchParam.longitude) + "&radius=" + String(yelpSearchParam.radius);
                Http.open("GET", url);
